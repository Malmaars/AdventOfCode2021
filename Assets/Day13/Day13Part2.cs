...
public class Day13Part2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            for(int k = 0; k < dotsArray.GetLength(1); k++)
            {
                if (dotsArray[i, k])
                    Instantiate(black, new Vector2(i, k), new Quaternion(0, 0, 0, 0));

                else
                    Instantiate(white, new Vector2(i, k), new Quaternion(0, 0, 0, 0));
            }
