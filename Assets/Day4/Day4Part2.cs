...
public class Day4Part2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        List<int> completedBoardIndex = new List<int>();
        ...
        for (int i = 0; i < pullingNumbers.Length; i++)
        {
            ...
            {
                if (completedBoardIndex.Contains(p))
                {
                    continue;
                }
                ...
                for (int r = 0; r < 5; r++)
                {
                    ...
                    {
                        //add the board to the list of completed boards.
                        completedBoardIndex.Add(p);
                        int totalPoints = 0;

                        if (completedBoardIndex.Count == intArraysList.Count)
                        {
                            //this is the one. this is the last board.
                            //And the last winning board is... intArraysList[p]
                            for (int u = 0; u < 5; u++)
                            {
                                for (int q = 0; q < 5; q++)
                                {
                                    if (!gotTheNumbers[p, u, q])
                                    {
                                        totalPoints += intArraysList[p][u, q];
                                    }
                                }
                            }
