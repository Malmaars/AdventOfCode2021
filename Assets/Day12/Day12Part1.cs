...
public class Day12Part1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach(string currentConnection in allCavesAsCaves[caveIndexes["start"]].Routes)
        {
            ...
            CheckNextCave(allCavesAsCaves[caveIndexes[currentConnection]], newRoute);

        }

        Debug.Log(possibleRoutes.Count);
    }

    public void CheckNextCave(cave currentCave, List<cave> caveRoute)
    {
        ...
        foreach(string nextCave in currentCave.Routes)
        {
            ...
            if(!newCave.Visited && newCave.Name != "start")
            {
                CheckNextCave(newCave, newBranch);
            }
